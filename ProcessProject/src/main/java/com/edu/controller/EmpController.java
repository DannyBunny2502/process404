...
public class EmpController {
	...
	*/
	 ...
	 * getUpdate(@RequestParam("notice_code") String notice_code, Model model)
	 * throws Exception {
	 * 
	 * BoardVO view = null; view = service.view(notice_code);
	 * 
	...
	// 사원코드 확인 후 회원정보 수정 할 페이지로 넘김
	 ...
	 public String getCode(@RequestParam("emp_code") String emp_code, Model model, HttpServletRequest request) throws  Exception { 
		emp_code = request.getParameter("emp_code"); 
		 
		EmployeeDTO join = null; // 사원코드 값 담을 empty 변수
		join = service.join(emp_code); // 사원코드 담아서 join 화면으로 넘기기 
