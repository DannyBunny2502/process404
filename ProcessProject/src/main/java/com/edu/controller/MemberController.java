...
public class MemberController {
	...
	public String memberList(Model model
		...
		List<MemberVO> list = memberService.memberList(search);
		List<MemberVO> modal = memberService.modalmemberList(search);
	...
	public String memberInsert(@ModelAttribute MemberVO vo){
		...
		// * (/)의 유무에 차이
		// /member/list : 루트 디렉토리를 기준
		// member/list : 현재 디렉토리를 기준
	...
	public String memberUpdate(@ModelAttribute MemberVO vo, Model model){
		// 비밀번호 체크
		boolean result = memberService.checkPw(vo.getId(), vo.getPassword());
		if(result){ // 비밀번호가 일치하면 수정 처리후, 전체 회원 목록으로 리다이렉트
			memberService.updateMember(vo);
			return "redirect:/member/list";
		} else { // 비밀번호가 일치하지 않는다면, div에 불일치 문구 출력, viwe.jsp로 포워드
			MemberVO vo2 = memberService.viewMember(vo.getId());
			vo.setHire_date(vo2.getHire_date());
